...
THOUGHT_SCHEMA = {
    ...
    "description": "The causal structure of what the person is working on. Labels are "
                   ...
                   "come from the Thought DNA vocabulary. Write them in the person's "
                   "own words and own language: labels are compared as meaning across "
                   "one index, not as spelling, so the same idea meets across "
                   "languages. A label that was translated first carries the "
                   "assistant's phrasing, and it is that phrasing which then gets "
                   "matched. A script the index cannot compare is refused by "
                   "prepare_thought, which names it. The conversation itself is "
                   "unaffected: only these labels are indexed.",
...
TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "reasoned (their claims, their causes and effects, in their own words and "
            "language), extracted from their own words. `context` (their raw sentences, "
            "≤ 4000 chars) is "
            "the fallback for English text that already carries explicit connectives "
            "(because, leads to, prevents, requires); a rule-based extractor reads it and "
            "abstains on prose without them. That extractor is English-only: a "
            "conversation in another language is served by the `thought` graph in that "
            "language, not by translating the prose to reach the extractor. `topic` and `domain` name the thought for other people in place of "
        ...
        if method == "initialize":
            ...
            return _result(msg_id, {
                ...
                "instructions": (
                    ...
                    "in it.\n\n"
                    "One index, one meaning. Write every node label in the person's "
                    "OWN words and own language, in short noun phrases — matching "
                    "compares meaning as well as shape, and it compares across "
                    "languages, so two people who share a thought in different "
                    "languages do meet. Do NOT translate their reasoning into English "
                    "first: a translation is your phrasing, not theirs, and it is your "
                    "choices that then get matched. If a script genuinely cannot be "
                    "compared, the server refuses and says which. Speak to the person "
                    "in their own language throughout, and translate what comes back "
