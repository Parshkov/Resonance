...
def main() -> int:
    ...
    with sync_playwright() as p:
        launch = {"args": ["--enable-features=WebMCP,WebMCPTesting"] + [a for a in os.environ.get("CHROME_EXTRA_ARGS", "").split() if a]}
        if os.environ.get("CHROME_PROXY"):
            launch["proxy"] = {"server": os.environ["CHROME_PROXY"]}
        if args.exe:
        ...
        ev["native_header_consent"] = text(pg, "#header-consent")  # evidence copy: plain-Chromium consent pill on first load
        print(f"[INFO] native (no shim) first load: header-consent={ev['native_header_consent']!r} status-pill={ev['native_status_pill']!r}")
