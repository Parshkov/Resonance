...
def main() -> int:
    ...
    with sync_playwright() as p:
        launch = {"args": ["--enable-features=WebMCP,WebMCPTesting"] + [a for a in os.environ.get("CHROME_EXTRA_ARGS", "").split() if a]}  # evidence copy: container egress
        if os.environ.get("CHROME_PROXY"):
            launch["proxy"] = {"server": os.environ["CHROME_PROXY"]}  # evidence copy: container egress
        if args.exe:
