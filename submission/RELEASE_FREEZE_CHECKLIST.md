...
- Delete the ten pre-existing duplicate guest sessions listed in `submission/evidence/public-origin-0aea577/SUMMARY.md`. **The tool now exists** (`RESONANCE_PURGE_SESSIONS=<id>[,<id>…]`, #172) because `purge-demo` selects by `record_kind` and these are real `volunteer` rows, while the product's `delete_session` needs the long-gone owner's token. **The production run has not happened yet.** Sequence: deploy → set the variable with the ten ids, keeping `ses-a95528cc2a90ef11` and `ses-099c77441b96db62` (the A/A' pair) → redeploy → check the log and health → unset → re-run `abc_mcp_test.py`. Their cost is measured, not assumed: on `8670568` the genuine cross-domain analogy is still at **rank 4** behind three exact copies scoring structural 1.0.
...
- Finish the custom domain **`resonance.parshkov.com`**. Railway custom domain `da441e84-…` is attached and the Squarespace `CNAME resonance -> ositddso.up.railway.app` is propagated with no CAA restriction, but the certificate was still `VALIDATING_OWNERSHIP` at the end of this run, so the host does not serve yet. `PUBLIC_ORIGIN` is already the new domain and `EXTRA_ORIGINS` keeps the platform host allowed, so the legacy URL is unaffected (verified: its RFC 9728/8414 metadata still names itself). Once it serves: re-run `oauth_smoke` and `abc_mcp_test` **against the new host** before any document calls it canonical.
