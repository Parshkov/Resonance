...
- **Browser WebMCP (competition eligibility):** the page registers native `document.modelContext` tools; a browser agent invokes them and the page visibly updates. The browser `resonance_prepare_thought` takes the agent's extracted `thought` (labelled causal graph) or raw `context`; it does **not** silently receive the user's whole conversation — the agent hands over exactly what the person approved, and the text itself is never retained. Without content it falls back to the thought visible on the page.
